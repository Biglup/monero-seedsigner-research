...
for 521 outputs 8.9 s, signing 2 inputs 4.5 s, 16 inputs 9.0 s, peak RSS 4 MB. A
521-output export is 57 KB (478 QR frames at 120 bytes each), its key image reply
50 KB, a 2-input unsigned tx 4.2 KB, its signed set 6.7 KB. The big export is a
one-time cost: Feather only exports outputs whose key images it does not know yet.

Rust on monero-oxide, about 1500 lines, stateless, seed only in RAM. Code, fixtures
with provenance and every raw measurement are in the repo. A research spike, not a
product; the camera round trip and SeedSigner UI come next.
